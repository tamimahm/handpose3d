...
for keys in set_key:
                ...
                for pth, label_data in label_dict.items():
                    ...
                    if label_key==activity_folder:
                        ...
                        if os.path.isdir(activity_folder_path) and 'keypoints' in os.listdir(activity_folder_path):
                            ...
                            if activity_folder.split('_')[2]=='right':
                                hand=1
                                if keys=='right':
                                    obj_dir=object_dir_ipsi
                                if keys=='top':
                                    obj_dir=object_dir_top
                                else:
                                    obj_dir=object_dir_contra
                            else:
                                hand=2
                                if keys=='left':
                                    obj_dir=object_dir_ipsi
                                if keys=='top':
                                    ...
                                    obj_dir=object_dir_contra
                            for fold in os.listdir(obj_dir):
                                if fold==label_data['patient_id']:
                                    obj_dir= os.path.join(obj_dir, label_data['patient_id'])
                                    for obj in os.listdir(obj_dir):
                                        if activity_folder.split('_')[4] in obj and 'Impaired' in obj:
                                            csv_file_path=os.path.join(obj_dir, obj)
                                            # Read the CSV file into a DataFrame
                                            df = pd.read_csv(csv_file_path)
                                            # Access the "y" column and save it in a variable
                                            obj_data = df['y']
                                    # Use regular expression to find numbers
                                    numbers = re.findall(r'\d+', activity_folder.split('_')[4])
                                    # Convert the found numbers to integers
                                    task= [int(num) for num in numbers]
                                                ...
                                                if key==keys and hand==1 :
                                                    if activity_folder.split('_')[4]=='activity1' or activity_folder.split('_')[4]=='activity2' or activity_folder.split('_')[4]=='activity3'or activity_folder.split('_')[4]=='activity4' or activity_folder.split('_')[4]=='activity5' or activity_folder.split('_')[4]=='activity6':
                                                        ...
                                                        if mat_data_[key].shape[0]<450 and np.abs(np.min(mat_data_[key])-np.max(mat_data_[key]))>0.1:
                                                            ...
                                                            obj_data = process_time_series(obj_data, target_length)/norm_dimension
                                                            ...
                                                            if nan_count<0.1*mat_data_[key].shape[0]:#consider missing 10% of the values then do manual interpolation
                                                                ...
                                                                data_label.append(lab_data/4)
                                                                hand_train.append(hand)
                                                                # Convert the found numbers to integers
                                                                task_id_train.append(task)
                                                                obj_data_train.append(obj_data)
                                                            ...
                                                            test_label.append(lab_data/4)
                                                            hand_test.append(hand)
                                                            # Convert the found numbers to integers
                                                            task_id_test.append(task)
                                                            obj_data_test.append(obj_data)
    ...
    feature_data=np.zeros([n_d,238,4])
    feature_test=np.zeros([n_t,238,4])
    feature_data[:,:,0]=data[:,:]
    for i in range(n_d):
        feature_data[i,:,1]=data_label[i]
        feature_data[i,:,2]=obj_data_train[i]
        feature_data[i,:,3]=task_id_train[i]
    feature_test[:,:,0]=test_data[:,:]
    for i in range(n_t):
        feature_test[i,:,1]=test_label[i]
        feature_test[i,:,2]=obj_data_test[i]
        feature_test[i,:,3]=task_id_test[i]
    ...
    for i in range(n_d):
        X[i,:,1]=data_label[i]
        # feature_data[i,:,1]=hand_train[i]
        X[i,:,3]=task_id_train[i]

    # y_intact, y, missing_mask_test, indicating_mask_test = mcar(feature_test, 0.3) # hold out 10% observed values as ground truth
    ...
    for i in range(len(imputation)):
        ...
        # ax1.plot(imputed, color='blue', marker='o',markersize=marker_size)
        # ax1.set_title(f'Imputed Data - Figure {i}'+title[i])
        
        # # Customize the second subplot (unimputed data)
        # ax2.plot(not_imputed, color='red', marker='o',markersize=marker_size)  # Example data for unimputed
